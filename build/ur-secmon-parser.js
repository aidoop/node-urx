...
class UrSecondaryMonitorParser {
        ...
        while (data.length > 0) {
            ...
            if (packetType === 16) {
                ...
                for (var ii = 0; ii < 6; ii++) {
                    tmpNames.push(`q_actual${ii}`, `q_target${ii}`, `qd_actual${ii}`, `I_actual${ii}`, `V_actual${ii}`, `T_motor${ii}`, `T_micro${ii}`, `jointMode${ii}`);
                }
                result['JointData'] = this.parsePacketData(packetData, '!iB dddffffB dddffffB dddffffB dddffffB dddffffB dddffffB', tmpNames);
            } //packetType === 1
            else if (packetType === 4) {
                if (semver_compare_1.default(this._version, '3.2') === -1) {
                ...
                let fmt = '';
                if (semver_compare_1.default(this._version, '3.0') >= 0) {
            ...
            else if (packetType === 8 && semver_compare_1.default(this._version, '3.2') >= 0) {
            ...
            else if (packetType === 7 && semver_compare_1.default(this._version, '3.2') >= 0) {
    ...
    getHeader(data) {
        let header = python_struct_1.default.unpack('!iB', data);
        return {
            packetSize: header[0],
            packetType: header[1]
        };
    }
    parsePacketData(data, fmt, names) {
        ...
        while (j < fmt.length && i < names.length) {
            ...
            if (/ |!|>|</.test(f)) {
                ...
                var fmtSize = python_struct_1.default.sizeOf(fmt[j]);
                if (formatData.length < fmtSize) {
                    throw `Error, length of data smaller than advertized: ${formatData.length}, ${fmtSize}, ${names}, ${f}, ${i}, ${j}`;
                }
                d[names[i]] = python_struct_1.default.unpack('!' + f, formatData.slice(0, fmtSize))[0];
        ...
        while (true) {
            if (data.length >= 5) {
                ...
                if (packetSize < 5 || packetSize > 2000 || packetType !== 16) {
                    ...
                    counter += 1;
                    if (counter > limit) {
                        console.log(`tried ${counter} times to find a packet in data, advertised packet size: ${packetSize}, type: ${packetType}`);
...
exports.UrSecondaryMonitorParser = UrSecondaryMonitorParser;
//# sourceMappingURL=ur-secmon-parser.js.map